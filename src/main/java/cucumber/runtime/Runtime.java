...
public class Runtime implements UnreportedStepExecutor {
    ...
    private FeatureImpl featureResult = null;
    ...
    public void buildBackendWorlds(Reporter reporter, Set<Tag> tags, Feature gherkinFeature, Scenario gherkinScenario) {
        ...
        skipNextStep = false;
        featureResult = new FeatureImpl(reporter, tags, gherkinFeature);
	...
	private void runStepHooks(Step step, @SuppressWarnings("rawtypes") List<HookDefinition> hookDefinitions, Reporter reporter, boolean isBefore) {
        if (!runtimeOptions.isDryRun()) {
            for (@SuppressWarnings("rawtypes") HookDefinition hookDefinition : hookDefinitions) {
